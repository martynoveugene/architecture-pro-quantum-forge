...
1. Был сделан запуск rag в тестовом режиме, который запускал вопросы, перечисленный в json-файле [dataset.json](task7/dataset.json)
и потом искал в ответе список обязательных слов и смотрел не ли в них запрещенных слов.
Если оба условия выполнены, то тест считался пройденным.
Но этот подход требовал точного совпадения слов и не позволял проверять фразы, например, 'информации в контексте нет', которая 
могла присутствовать, но быть в виде 'информации о таком-то герое в контексте нет', и это было сложно определить матчингом слов.
2. Поэтому была подключена модель-судья 'qwen2.5:7b', которая анализировала соответствие ответа эталонному ответу
и искала присутствие-отсутсвие слов с учетом падежей и склонений.
в 'dataset.json' был добавлен эталонный ответ, как его бы сформулировал человек - кратко и по делу.
Так же была протестирована модель-судья "IlyaGusev/saiga_llama3_8b", но она не запустилась из-за больших требований к памяти.
3. Лог тестирования собран в [test_log_2026-06-13.log](task7/test_log_2026-06-13.log)
Результаты тестирования собраны в [test_results_2026-06-13.json](task7/test_results_2026-06-13.json)
4. Схема тестировани и работы RAG доступна в ![test_schema.png](task7/test_schema.png)
